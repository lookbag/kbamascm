        ...
        const DB = {
            async get(table) {
                ...
                catch (err) { console.error(`[DB.get:${table}] 오염된 로컬 데이터 발견. 자동 초기화합니다.`); localStorage.removeItem(`scm_${table}`); return []; }
            },
      ...
      <td>${actionBtn}</td>
        ...
        async function settingsRefreshStatus() {
            ...
            for (const [table, pk] of Object.entries(TABLE_PK)) {
                let local = [];
                try {
                    local = JSON.parse(localStorage.getItem(`scm_${table}`) || '[]');
                } catch (e) {
                    console.error(`[Settings] ${table}의 오염된 데이터 발견. 자동 삭제합니다.`);
